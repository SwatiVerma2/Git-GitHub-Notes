...
2. To show the current state of the working directory and the staging area.
 ...
 Make few changes in the file in vs code then save it. It will show the files with yellow color and ‘M’ → modified

The **staging area** in Git, also known as the index, is an intermediate space where changes are gathered before they are committed to the repository. 
